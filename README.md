...
<img src="Imagenes\Escalamiento.png">
...
Se puede observar un correcto funcionamiento en la transición de las imágenes, ya que, a partir de la entrada `visua`, se realiza el cambio de imagen a través de los diferentes estados de `fms_state` (0: IDLE, 1: HAMBRIENTO, etc.). Para evitar que las imágenes se superpongan, se utiliza el parámetro `transmission_Done`, que asegura que una imagen se complete completamente en pantalla antes de pasar a la siguiente. Esto se evidencia en la simulación, con el parámetro visua cambiando rápidamente entre los valores 0 y 3. Sin embargo, solo cuando la señal transmission_Done está activa (en 1), el valor en `fms_state` cambia a 3. Esto indica que primero se transmite la imagen correspondiente al estado 0, y una vez finalizada su transmisión, se procede con la imagen del estado 3. De esta manera, se garantiza que la imagen del estado 0 se complete antes de realizar la transición a la siguiente imagen.
